...
const Component = forwardRef<HTMLElement, StickyScrollProps>(({ className, ...props }, ref) => {
  ...
  function FillImage({
    ...
    return (
      <div className="relative h-full w-full overflow-hidden rounded-none border-[3px] border-white/20 transition-colors duration-500 ease-out group-hover:border-violet-300/55">
        <Image
          src={src}
          alt={alt}
          fill
  ...
  function TileLink({
    ...
    return (
      <a
        ...
        target="_blank"
        rel="noreferrer"
        className="group relative z-0 block h-full w-full cursor-pointer rounded-none outline-none transition-transform duration-500 ease-out will-change-transform hover:z-20 hover:scale-[1.03] focus-visible:ring-2 focus-visible:ring-violet-500"
      >
        {children}
      </a>
    );
  ...
  return (
    <section
      ...
      {...props}
    >
      <div className="flex flex-col md:grid md:grid-cols-12 gap-3 md:gap-2 overflow-visible px-4 md:px-0">
        <div className="flex flex-col gap-3 md:grid md:gap-2 md:col-span-4 overflow-visible">
          {leftTiles.map((t, idx) => (
            <figure key={`${t.src}:${idx}`} className="relative w-full h-[250px] md:h-96 overflow-visible">
        ...
        <div
          className="flex flex-col gap-3 md:sticky md:top-0 md:h-screen w-full md:col-span-4 md:grid md:gap-2 overflow-visible"
          style={{ gridTemplateRows: `repeat(${Math.max(1, stickyTiles.length)}, minmax(0, 1fr))` }}
        >
          {stickyTiles.map((t, idx) => (
            <figure key={`${t.src}:sticky:${idx}`} className="relative w-full h-[250px] md:min-h-0 md:h-full overflow-visible">
              <TileLink href={t.href}>
        ...
        <div className="flex flex-col gap-3 md:grid md:gap-2 md:col-span-4 overflow-visible">
          {rightTiles.map((t, idx) => (
            <figure key={`${t.src}:right:${idx}`} className="relative w-full h-[250px] md:h-96 overflow-visible">
              <TileLink href={t.href}>
                <FillImage src={t.src} alt={t.alt} />
              </TileLink>
            </figure>
